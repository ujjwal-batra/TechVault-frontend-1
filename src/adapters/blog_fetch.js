  ...
  async function postBlogRequest(details, link) {
    await axios({
      ...
      for (var i = 0; i < res.data.length; i++) {
        var companyLink = res.data[i].link.substring(
          8,
          res.data[i].link.length
        );
        var imgLink = "";
        for (let j = 0; j < companyLink.length; j++) {
          if (companyLink[j] === "/") break;
        ...
        if (imgLink === "tech.ebayinc.comhttps:") imgLink = "tech.ebayinc.com";
        imgLink = "//logo.clearbit.com/" + imgLink + "?size=180";
        var x = {
          id: res.data[i].id,
  ...
  function fetchMoreListItems() {
    ...
    try {
      ...
      // scrolled post request
        ...
        var details;
        if (prevLink === "http://techvault.tech:8080/search/keyword/")
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (
        ...
        link = "http://techvault.tech:8080/home/" + fromSiblings.selevtedNav;
        post_req = false;
        setPostReq(false);
        setLoadNo(1);
      } else if (
        ...
        link = "http://techvault.tech:8080/search/company/";
        post_req = true;
        setPostReq(true);
        setLoadNo(1);
      } else if (
        ...
        link = "http://techvault.tech:8080/search/keyword/";
        post_req = true;
        setPostReq(true);
        setLoadNo(1);
      } else {
      ...
      // initial post request
        ...
        setPrevLink(link);
        var details;
        if (link === "http://techvault.tech:8080/search/keyword/")
          details = { searchWords: fromSiblings.passedTopics };
        else details = { searchWords: fromSiblings.passedCompany };
        postBlogRequest(details, link);
      }
